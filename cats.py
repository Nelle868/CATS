...
def pick(paragraphs: list[str], select, k: int) -> str:
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    chosen_p = [p for p in paragraphs if select(p)]
    if k < len(chosen_p): 
        return chosen_p[k]
    return ''
    # END PROBLEM 1
...
def about(keywords: list[str]):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    # To return a function, one needs to be created: 
    def contains_keywords(some_paragraph): 
        each_word = split(remove_punctuation(lower(some_paragraph)))
        for i in each_word: 
            if i in keywords: 
                return True
        return False
...
def accuracy(typed: str, source: str) -> float:
    ...
    return (words_typed_correctly / len(typed_words)) * 100
    # END PROBLEM 3
...
def wpm(typed: str, elapsed: int) -> float:
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    # Divide the total number of characters 
    # typed (including spaces) by 5 
    # (the average word length) and then 
    # dividing the result by the elapsed time 
    # in minutes, therefore: 
    # WPM = (words typed/5) / (elapsed time/60)
    letter_count = len(typed)
    words_typed = letter_count / 5
    mins = elapsed / 60

    wpm = words_typed / mins
    return wpm
...
def autocorrect(typed_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    if typed_word in word_list or not word_list: 
        return typed_word
    best_match = min(word_list, key = lambda x: diff_function(typed_word, x, limit))
    return best_match if limit >= diff_function(typed_word, best_match, limit) else typed_word
...
def report_progress(typed: list[str], source: list[str], user_id: int, upload) -> float:
    ...
    for i in range(len(typed)): 
        if source [i] == typed [i]: 
            ...
            break
    current_progress = whats_correct / len(source)
    upload({"id": user_id, "progress": current_progress})
...
def time_per_word(words: list[str], timestamps_per_player: list[list[int]]) -> dict:
    ...
    for user in tpp: 
        ...
        for i in range(1, len(user)): 
            users_time.append(user[i] - user[i - 1])
        times.append(users_time)
    # END PROBLEM 9
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    end_result = [[] for _ in player_indices]
    for j in word_indices: 
        best_user = min(player_indices, key = lambda i: get_time(times, i, j))
        best_time = get_time(times, best_user, j)
        end_result[best_user].append(words[j])
    return end_result
